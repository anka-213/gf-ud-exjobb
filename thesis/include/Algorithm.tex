...
\begin{verbatim}
    ...
    NOUN -> N
\end{verbatim}
And then we use the GF parser on each individual lemma (word in the dictionary form), with the category or categories we just got\footnote{The GF parser needs to know which category the result should be in order to be able to parse correctly.}. % \todo{Explain that gf parser wants to know which category to parse in}.
...
\begin{figure}[H]
    ...
    \subcaptionbox{The black cat : NP\label{ci3:the_black_cat_NP}}
        {\includegraphics[scale=0.75]{figure/black_cats/the_black_cat_NP_gf.eps}}
