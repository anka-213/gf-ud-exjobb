...
\begin{figure}
    ...
    \begin{dependency}
        ...
        \draw [->, very thick, dashed, gray] (arg1) -- node[left] {det} (\wordref{1}{1});
        \draw [->, very thick, dashed, gray] (arg2) -- node[right] {head} (\wordref{1}{2});
    \end{dependency} \\
...
% Similarly to an ordinary bottom up parser, we annotate each word with one or several GF trees. However, In contrast with ordinary bottom-up parsers, we don't start with a linear string, but instead start with a UD tree, where each node corresponds to a word,
...
\begin{figure}[H]
    ...
    \begin{dependency}
        ...
        \draw [->, very thick, dashed, gray] (f2arg1) -- node[left, yshift = -3ex] {amod} (\wordref{2}{2});
